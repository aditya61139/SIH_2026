...
export const LiveCallMonitor: React.FC<LiveCallMonitorProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-2xl bg-white p-4 sm:p-5 border border-[#E7E2DA] shadow-xs">
        <div className="flex items-center space-x-3.5">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#FFF1E8] border border-[#E7E2DA]">
            <Radio className="h-6 w-6 text-[#C2410C]" />
          </div>
          <div>
          ...
          <button
            ...
            className={`flex w-full sm:w-auto items-center justify-center space-x-2 rounded-xl px-5 py-3 text-sm font-bold transition-all shadow-md ${
              isMonitoring
                ...
                : 'bg-[#C2410C] hover:bg-[#9A3412] text-white shadow-[#C2410C]/25'
            }`}
          >
            {isMonitoring ? (
              <>
      ...
      {errorMessage && (
        <div className="rounded-xl bg-red-50 p-4 border border-red-200 text-[#DC2626] text-xs font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="rounded-2xl bg-white p-5 border border-[#E7E2DA] shadow-xs">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2">
                <Mic className={`h-4 w-4 ${isMonitoring ? 'text-[#C2410C] animate-pulse' : 'text-[#94A3B8]'}`} />
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className={`inline-flex h-2 w-2 rounded-full ${isMonitoring ? 'bg-[#15803D] animate-ping' : 'bg-[#94A3B8]'}`} />
              </div>
            </div>

            {/* Canvas Visualizer */}
            <div className="relative overflow-hidden rounded-xl bg-[#FDFBF7] border border-[#E7E2DA] h-40">
              <canvas
                ref={canvasRef}
                width={640}
                height={160}
              ...
              />
              {!isMonitoring && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#FDFBF7]/90 text-center p-4">
                  <Mic className="h-7 w-7 text-[#94A3B8] mb-1.5" />
            ...
            <div className="mt-3 flex items-center space-x-3">
              <Volume2 className="h-4 w-4 text-[#64748B] shrink-0" />
              <div className="flex-1 h-2 rounded-full bg-[#ECE8E1] overflow-hidden border border-[#E7E2DA]">
                <div
                  className="h-full bg-gradient-to-r from-[#C2410C] to-[#15803D] transition-all duration-75"
          ...
          <div className="rounded-2xl bg-white p-5 border border-[#E7E2DA] shadow-xs">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[11px] text-[#64748B] font-mono">Sliding Window: 2.0s</span>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {/* Layer 1: Spectral */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L1: Spectral</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#1E293B]">
                  {Math.round(layerScores.spectral * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">Flatness & Phase</span>
              </div>

              {/* Layer 2: Prosody */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L2: Prosody</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#1E293B]">
                  {Math.round(layerScores.prosody * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">F0 Pitch Variance</span>
              </div>

              {/* Layer 3: Breathing */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L3: Breathing</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#1E293B]">
                  {Math.round(layerScores.breathing * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">Respiration Gaps</span>
              </div>

              {/* Layer 4: Vocoder */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L4: Vocoder</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#1E293B]">
                  {Math.round(layerScores.acoustic_artifacts * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">Filter Cutoff</span>
              </div>

              {/* Layer 5: LFCC */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L5: ASVspoof LFCC</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#C2410C]">
                  {Math.round(layerScores.lfcc * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">Linear Cepstrum ΔΔ</span>
              </div>

              {/* Layer 6: Glottal */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L6: Glottal Flow</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#C2410C]">
                  {Math.round(layerScores.glottal * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">LPC-NAQ Biomechanics</span>
              </div>

              {/* Layer 7: Perturbation */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L7: Perturbation</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#C2410C]">
                  {Math.round(layerScores.perturbation * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">Jitter & Shimmer</span>
              </div>

              {/* Layer 8: Bispectrum */}
              <div className="rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA]">
                <span className="text-[10px] text-[#64748B] uppercase font-semibold block">L8: Bispectrum</span>
                <div className="mt-0.5 text-base font-mono font-bold text-[#C2410C]">
                  {Math.round(layerScores.bispectrum * 100)}%
                </div>
                <span className="text-[9px] text-[#94A3B8] block truncate">QPC Phase Coupling</span>
              </div>
            </div>

            {/* Speaker Diarization Badge */}
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between rounded-xl bg-[#F8F5EF] p-3 border border-[#E7E2DA] text-xs">
              <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="mt-2 sm:mt-0 text-[11px] font-semibold text-[#C2410C] hover:underline"
              >
        ...
        <div className="lg:col-span-5 space-y-6 flex flex-col">
          ...
          <div className="rounded-2xl bg-white p-4 border border-[#E7E2DA] shadow-xs flex items-center justify-center">
            <ForensicRadar
              scores={layerScores}
              riskLevel={riskLevel}
            />
